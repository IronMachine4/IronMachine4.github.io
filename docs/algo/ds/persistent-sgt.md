...
=== "建树"
    ...
    ```cpp
    int build(int l,int r)//对区间 [l,r] 建树，返回树根
    {
        ++tot;//动态开点
        if(l==r) return tot;
        int mid=(l+r)/2;
...
=== "单点修改"
    ...
    ```cpp
    int modify(int o,int l,int r,int p,int c)//将 a[p] 改为 c，返回 [l,r] 新版本的树根
    {
        o=copy(o);//o 的节点信息一定会被修改，所以复制一份作为新版本的树根
        if(l==r)
        {
            t[o]=c;
            return o;
...
=== "区间查询"
    ...
    ```cpp
    //注意：在修改操作时，我们返回了修改后新版本的树根。则查询函数 o 的初始值为你要查询的版本的树根
    int query(int o,int l,int r,int L,int R) //很普通的查询操作
    {
...
???+ note "[P3834 【模板】可持久化线段树 2](https://www.luogu.com.cn/problem/P3834)"
